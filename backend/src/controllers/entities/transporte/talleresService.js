...
async function getAllTalleres() {
  try {
    const result = await query(
      'SELECT id, nombre, celular, direccion, sector, estado, "PrecioCarro", "PrecioMoto" FROM talleres ORDER BY nombre'
    );
    return result.rows;
  } catch (error) {
...
async function getTallerById(id) {
  try {
    const result = await query(
      'SELECT id, nombre, celular, direccion, sector, estado, "PrecioCarro", "PrecioMoto" FROM talleres WHERE id = $1',
      [id]
    );
...
async function createTaller(data) {
  try {
    await query(
      'INSERT INTO talleres (id, nombre, celular, direccion, sector, estado, "PrecioCarro", "PrecioMoto") VALUES ($1, $2, $3, $4, $5, $6, $7, $8)',
      [
        data.id,
        data.nombre,
        data.celular || '',
...
async function updateTaller(id, data) {
  try {
    ...
    if (data.PrecioCarro !== undefined) { updates.push('"PrecioCarro" = $' + i++); values.push(data.PrecioCarro); }
    if (data.PrecioMoto  !== undefined) { updates.push('"PrecioMoto" = $'  + i++); values.push(data.PrecioMoto); }
