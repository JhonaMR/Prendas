...
/**
 * Ajustar valor para que termine en 900
 */
...
const calcularTotales = (secciones) => {
    ...
    // Costo para contabilizar (sin provisiones)
...
/**
 * Calcular precio de venta desde rentabilidad
 */
const calcularPrecioVenta = (costoTotal, rentabilidad) => {
    return ajustarA900(costoTotal * (1 + rentabilidad / 100));
};

/**
 * Calcular rentabilidad desde precio de venta
 */
...
/**
 * Calcular margen ganancia cliente (35%)
 */
const calcularMargenGanancia = (precioVenta) => {
 ...
 * Obtener todas las fichas de costo
 */
 ...
 * Obtener una ficha de costo específica con sus cortes
 */
const getFichaCosto = async (req, res) => {
    try {
        ...
        // Obtener cortes
        const cortesResult = await pool.query(`
            SELECT *
            FROM fichas_cortes
            WHERE ficha_costo_id = $1
