...
const SubidaFotos: React.FC<SubidaFotosProps> = ({
  ...
  const handleFileSelect = async (file: File, numero: 1 | 2) => {
    ...
    const nuevoNombre = numero === 1 
      ? `${referencia}.${extension}`
      : `${referencia}-2.${extension}`;
    ...
    try {
      ...
      if (result.success) {
        if (numero === 1) {
          onFoto1Change(result.data.path);
        } else {
          onFoto2Change(result.data.path);
        }
        console.log('✅ Foto subida:', result.data.path);
      } else {
  ...
  const FotoBox = ({ 
    ...
    const imageSrc = preview || (foto ? `${baseUrl}${foto}` : null);
          ...
          <div className="mt-3">
            <input
              ...
              onChange={(e) => {
                ...
                if (file) handleFileSelect(file, numero);
              }}
              className="hidden"
              id={`foto-${numero}`}
            />
            <label
              htmlFor={`foto-${numero}`}
              className="block w-full py-2 px-4 bg-gradient-to-r from-blue-600 to-indigo-600 text-white font-black rounded-xl text-center cursor-pointer hover:shadow-lg transition-all text-sm uppercase tracking-wider"
            >
              {imageSrc ? 'Cambiar' : 'Seleccionar'} Foto {numero}
            </label>
    ...
    <div className="bg-white p-6 rounded-3xl border border-slate-100 shadow-sm">
      <div className="mb-4">
        ...
        <p className="text-xs text-slate-400 font-bold mt-1">
          JPG, JPEG o PNG • Máximo 5MB por foto
        </p>
      </div>
