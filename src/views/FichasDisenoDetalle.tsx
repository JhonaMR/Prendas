...
const FichasDisenoDetalle: React.FC<Props> = ({ state, user, updateState, onNavigate, params }) => {
    ...
    const handleGuardar = async () => {
        if (!isDisenadora) { alert('No tienes permisos para guardar fichas de diseño'); return; }
        ...
        <div className="space-y-6 pb-20">
            ...
            <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
                <div className="space-y-6">
                    <SubidaFotos referencia={referencia} foto1={foto1} foto2={foto2} onFoto1Change={mark(setFoto1)} onFoto2Change={mark(setFoto2)} readOnly={!isDisenadora} />
                    <div className="bg-white p-6 rounded-3xl border border-slate-100 shadow-sm space-y-4">
                        ...
                        {[['Descripción de Prenda', descripcion, mark(setDescripcion)], ['Marca', marca, mark(setMarca)], ['Novedad o Correría', novedad, mark(setNovedad)]].map(([label, val, setter]: any) => (
                            <div key={label as string}>
                                ...
                                <input type="text" value={val} onChange={e => setter(e.target.value)} readOnly={!isDisenadora} className="w-full px-4 py-3 bg-slate-50 border-2 border-slate-200 rounded-xl font-bold focus:ring-4 focus:ring-pink-100 focus:border-pink-500" />
                            </div>
                        ))}
                        <div className="grid grid-cols-2 gap-4">
                            {[['Muestra #1', muestra1, mark(setMuestra1)], ['Muestra #2', muestra2, mark(setMuestra2)]].map(([label, val, setter]: any) => (
                                <div key={label as string}><label className="text-[10px] font-black text-slate-400 uppercase tracking-widest block mb-2">{label}</label><input type="text" value={val} onChange={e => setter(e.target.value)} readOnly={!isDisenadora} className="w-full px-4 py-3 bg-slate-50 border-2 border-slate-200 rounded-xl font-bold focus:ring-4 focus:ring-pink-100 focus:border-pink-500" /></div>
                            ))}
                        </div>
                        <div>
                            <label className="text-[10px] font-black text-slate-400 uppercase tracking-widest block mb-2">Observaciones</label>
                            <textarea value={observaciones} onChange={e => { setObservaciones(e.target.value); setHasUnsavedChanges(true); }} readOnly={!isDisenadora} rows={4} className="w-full px-4 py-3 bg-slate-50 border-2 border-slate-200 rounded-xl font-bold focus:ring-4 focus:ring-pink-100 focus:border-pink-500 resize-none" />
                        </div>
                    </div>
                ...
                <div className="space-y-6">
                    <SeccionConceptos titulo="MATERIA PRIMA" color="pink" conceptos={materiaPrima} onChange={mark(setMateriaPrima)} readOnly={!isDisenadora} mostrarTipo={true} />
                    <SeccionConceptos titulo="MANO DE OBRA" color="blue" conceptos={manoObra} onChange={mark(setManoObra)} readOnly={!isDisenadora} />
                    <SeccionConceptos titulo="INSUMOS DIRECTOS" color="slate" conceptos={insumosDirectos} onChange={mark(setInsumosDirectos)} readOnly={!isDisenadora} />
                    <SeccionConceptos titulo="INSUMOS INDIRECTOS" color="orange" conceptos={insumosIndirectos} onChange={mark(setInsumosIndirectos)} readOnly={!isDisenadora} />
                    <SeccionConceptos titulo="PROVISIONES" color="red" conceptos={provisiones} onChange={mark(setProvisiones)} readOnly={!isDisenadora} />
                </div>
            </div>
