...
const FichasDisenoDetalle: React.FC<Props> = ({ state, user, updateState, onNavigate, params }) => {
    ...
    const handleConfirmarDuplicar = async () => {
        ...
        setDuplicando(true);
        try {
            const res = await apiFichas.duplicarFichaDiseno(
                referencia,
                referenciaDuplicar.trim().toUpperCase(),
                selectedDesigner
            );
            if (res.success) {
                alert('✅ Ficha de diseño duplicada exitosamente');
                const fichas = await apiFichas.getFichasDiseno();
                updateState(prev => ({ ...prev, fichasDiseno: fichas }));
                setShowModalDuplicar(false);
                onNavigate('fichas-diseno-detalle', { referencia: referenciaDuplicar.trim().toUpperCase() });
            } else {
                ...
                <div className={`fixed inset-0 backdrop-blur-sm z-50 flex items-center justify-center p-4 ${isDark ? 'bg-slate-900/60' : 'bg-slate-900/40'}`}>
                    <div className={`rounded-3xl shadow-2xl max-w-md w-full p-8 transition-colors duration-300 ${isDark ? 'bg-[#4a3a63] border border-violet-700' : 'bg-white'}`}>
                        <div className="text-center mb-6">
                            <h3 className={`text-2xl font-black mb-2 ${isDark ? 'text-violet-50' : 'text-slate-800'}`}>Duplicar Ficha de Diseño</h3>
                            <p className={`text-sm font-bold ${isDark ? 'text-violet-300' : 'text-slate-500'}`}>Copia la ficha de diseño a una nueva referencia sin fotos.</p>
                        </div>
                        ...
                        <div className="flex gap-3 mt-8">
                            ...
                            <button 
                                ...
                                className="flex-1 px-6 py-3 bg-gradient-to-r from-pink-600 to-pink-500 text-white font-black rounded-xl hover:shadow-lg transition-all uppercase tracking-wide text-sm disabled:opacity-50 disabled:cursor-not-allowed"
                            >
