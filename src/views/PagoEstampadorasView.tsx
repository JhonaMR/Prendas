...
const PagoEstampadorasView: React.FC<Props> = ({ state, onNavigate, onBack }) => {
  ...
  return (
    <div className={`pb-24 space-y-6 transition-colors duration-300 ${isDark ? 'bg-[#3d2d52]' : ''}`}>
      ...
      {/* Header */}
      <div className={`relative rounded-3xl p-6 overflow-hidden shadow-lg transition-colors duration-300 ${isDark ? 'bg-gradient-to-r from-violet-600 via-purple-600 to-violet-600' : 'bg-gradient-to-r from-purple-500 via-violet-400 to-purple-400'}`}>
        ...
        <div className="relative flex items-center justify-between flex-wrap gap-3">
          <div className="flex items-center gap-3">
            ...
            <div>
              ...
              <p className={`text-sm transition-colors duration-300 ${isDark ? 'text-violet-100' : 'text-purple-100'}`}>Liquidación de lotes por estampado y aplique</p>
            </div>
          </div>
          <button onClick={() => setConfigOpen(true)} className={`flex items-center gap-2 px-4 py-2 rounded-xl text-sm font-semibold transition-colors backdrop-blur-sm ${isDark ? 'bg-violet-500/30 hover:bg-violet-500/50 text-violet-100' : 'bg-white/20 hover:bg-white/30 text-white'}`}>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-2 gap-4 items-start">
        ...
        <div className={`rounded-3xl border shadow-sm overflow-hidden transition-colors duration-300 ${isDark ? 'bg-[#4a3a63] border-violet-700' : 'bg-white border-slate-100'}`}>
          <div className={`px-6 py-3 border-b transition-colors duration-300 ${isDark ? 'bg-[#5a4a75] border-violet-600' : 'bg-purple-50 border-purple-100'}`}>
            <p className={`text-sm font-black uppercase tracking-widest text-center transition-colors duration-300 ${isDark ? 'text-violet-200' : 'text-slate-800'}`}>Buscar referencia</p>
          </div>
          <div className="p-6 space-y-4">
            <div className="grid grid-cols-2 gap-3">
              <div>
                ...
                <div className="flex gap-2">
                  <input type="text" value={referenciaInput} onChange={e => setReferenciaInput(e.target.value.toUpperCase())} onKeyDown={handleKeyDown} placeholder="Ej: 13121"
                    className={`flex-1 px-4 py-3 rounded-2xl border font-bold text-sm transition-all outline-none ${isDark ? 'bg-[#3d2d52] border-violet-600 text-violet-100 placeholder-violet-600 focus:ring-2 focus:ring-violet-400 focus:border-violet-500' : 'border-slate-200 focus:ring-2 focus:ring-purple-200 focus:border-purple-400 text-slate-800'}`} />
                  <button onClick={buscarReferencia} className={`px-5 py-3 rounded-2xl font-bold text-sm transition-colors shadow-sm ${isDark ? 'bg-violet-600 hover:bg-violet-700 text-white' : 'bg-purple-500 hover:bg-purple-600 text-white'}`}>Buscar</button>
                </div>
                {notFound && (
              ...
              <div>
                <label className={`text-xs font-bold mb-1.5 block uppercase tracking-wider transition-colors duration-300 ${isDark ? 'text-violet-300' : 'text-slate-500'}`}>Unidades</label>
                <input type="number" min={1} value={unidades} onChange={e => setUnidades(e.target.value === '' ? '' : Number(e.target.value))}
                  ...
                  className={`w-full px-4 py-3 rounded-2xl border font-bold text-sm text-center transition-all outline-none disabled:opacity-40 disabled:cursor-not-allowed ${isDark ? 'bg-[#3d2d52] border-violet-600 text-violet-100 placeholder-violet-600 focus:ring-2 focus:ring-violet-400 focus:border-violet-500' : 'border-slate-200 focus:ring-2 focus:ring-purple-200 focus:border-purple-400 text-slate-800'}`} />
              </div>
            </div>
            {/* Info de la ficha */}
            {ficha && (
              <div className={`pt-4 border-t space-y-1 transition-colors duration-300 ${isDark ? 'border-violet-700' : 'border-slate-100'}`}>
                <p className={`text-base font-black transition-colors duration-300 ${isDark ? 'text-violet-50' : 'text-slate-800'}`}>{ficha.referencia}</p>
                <div className="flex items-start justify-between gap-2">
                  ...
                  <div className="flex gap-1.5 flex-shrink-0 flex-wrap justify-end">
                    {ficha.marca && <span className={`text-xs font-bold px-2 py-0.5 rounded-lg transition-colors duration-300 ${isDark ? 'bg-violet-700/40 text-violet-200' : 'bg-purple-50 text-purple-500'}`}>{ficha.marca}</span>}
        ...
        <div className={`rounded-3xl border shadow-sm overflow-hidden transition-colors duration-300 ${isDark ? 'bg-[#4a3a63] border-violet-700' : 'bg-white border-slate-100'}`}>
          <div className={`px-6 py-3 border-b flex items-center justify-between transition-colors duration-300 ${isDark ? 'bg-[#5a4a75] border-violet-600' : 'bg-purple-50 border-purple-100'}`}>
              ...
              <div className="flex items-center gap-2">
                {ficha.foto1 && (
                  <button onClick={() => setFotoModal({ url: ficha.foto1!, ref: ficha.referencia })}
                    className={`p-1.5 rounded-lg transition-colors border ${isDark ? 'bg-violet-900/40 hover:bg-violet-900/60 text-violet-400 border-violet-600' : 'bg-white hover:bg-purple-100 text-purple-500 border-purple-100'}`}>
                    <svg xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24" strokeWidth={2} stroke="currentColor" className="w-4 h-4">
          ...
          <div className="p-6">
            {!ficha ? (
              <p className={`text-sm italic text-center py-8 transition-colors duration-300 ${isDark ? 'text-violet-400' : 'text-slate-400'}`}>Busca una referencia para ver sus conceptos.</p>
            ) : conceptosFiltrados.length === 0 ? (
              <p className={`text-sm italic text-center py-4 transition-colors duration-300 ${isDark ? 'text-violet-400' : 'text-slate-400'}`}>No se encontraron conceptos de estampado, aplique, sublimado, etc. en la ficha de costo.</p>
            ) : (
              <div className="space-y-2">
                <div className={`grid grid-cols-4 gap-3 px-4 pb-3 border-b transition-colors duration-300 ${isDark ? 'border-violet-700' : 'border-slate-100'}`}>
                  <span className={`text-xs font-black uppercase tracking-wider col-span-2 transition-colors duration-300 ${isDark ? 'text-violet-400' : 'text-slate-400'}`}>Concepto</span>
                ...
                {conceptosFiltrados.map((c, i) => (
                  <div key={i} className={`grid grid-cols-4 gap-3 px-4 py-3 rounded-2xl items-center transition-colors duration-300 ${isDark ? 'bg-violet-700/40 border border-violet-600' : 'bg-purple-50 border border-purple-100'}`}>
                    <div className="col-span-2 flex items-center gap-2">
                      <button onClick={() => agregarLote(c)}
                        className={`w-7 h-7 rounded-full flex items-center justify-center flex-shrink-0 transition-colors shadow-sm font-black text-sm ${isDark ? 'bg-violet-600 hover:bg-violet-700 text-white' : 'bg-purple-500 hover:bg-purple-600 text-white'}`}>
                        +
                      </button>
                      <span className={`text-sm font-bold transition-colors duration-300 ${isDark ? 'text-violet-100' : 'text-slate-700'}`}>{c.concepto}</span>
                    </div>
                    <span className={`text-[15px] font-semibold text-right leading-none transition-colors duration-300 ${isDark ? 'text-violet-400' : 'text-slate-500'}`}>{fmt(c.total)}</span>
                    <span className={`text-sm font-black text-right transition-colors duration-300 ${isDark ? 'text-violet-200' : 'text-purple-600'}`}>{fmt(c.total * (typeof unidades === 'number' ? unidades : 1))}</span>
                  </div>
                ))}
              </div>
            )}
      ...
      {/* Lotes a liquidar */}
      <div className={`rounded-3xl border shadow-sm overflow-hidden transition-colors duration-300 ${isDark ? 'bg-[#4a3a63] border-violet-700' : 'bg-white border-slate-100'}`}>
        <div className={`px-6 py-3 flex items-center justify-between relative transition-colors duration-300 ${isDark ? 'bg-violet-900/60' : 'bg-slate-800'}`}>
          <p className={`absolute left-1/2 -translate-x-1/2 text-sm font-black uppercase tracking-widest transition-colors duration-300 ${isDark ? 'text-violet-200' : 'text-white'}`}>Lotes a liquidar</p>
          <div className="ml-auto">
            <button onClick={agregarLoteManual}
              className={`flex items-center gap-1.5 text-xs font-bold px-3 py-1.5 rounded-lg transition-colors ${isDark ? 'text-violet-300 hover:text-violet-100 bg-violet-700/40 hover:bg-violet-700/60' : 'text-white/80 hover:text-white bg-white/10 hover:bg-white/20'}`}>
        ...
        <div className="p-6 space-y-2">
          {lotes.length === 0 ? (
            <p className={`text-center font-medium py-8 italic transition-colors duration-300 ${isDark ? 'text-violet-400' : 'text-slate-300'}`}>Agrega lotes desde los conceptos de la referencia buscada.</p>
          ) : (
            <>
            {/* Header columnas */}
            <div className={`grid grid-cols-12 gap-2 px-4 pb-3 border-b items-center transition-colors duration-300 ${isDark ? 'border-violet-700' : 'border-slate-100'}`}>
              <span className={`col-span-1 text-xs font-black uppercase tracking-wider text-center transition-colors duration-300 ${isDark ? 'text-violet-400' : 'text-slate-400'}`}>Cobro</span>
              <span className={`col-span-3 text-xs font-black uppercase tracking-wider transition-colors duration-300 ${isDark ? 'text-violet-400' : 'text-slate-400'}`}>Concepto</span>
              <span className={`col-span-1 text-xs font-black uppercase tracking-wider text-center transition-colors duration-300 ${isDark ? 'text-violet-400' : 'text-slate-400'}`}>Ref.</span>
              <span className={`col-span-2 text-xs font-black uppercase tracking-wider text-right transition-colors duration-300 ${isDark ? 'text-violet-400' : 'text-slate-400'}`}>Vlr. Unit.</span>
              <span className={`col-span-1 text-xs font-black uppercase tracking-wider text-center transition-colors duration-300 ${isDark ? 'text-violet-400' : 'text-slate-400'}`}></span>
              <span className={`col-span-1 text-xs font-black uppercase tracking-wider text-center transition-colors duration-300 ${isDark ? 'text-violet-400' : 'text-slate-400'}`}>Und.</span>
              <span className={`col-span-1 text-xs font-black uppercase tracking-wider text-right transition-colors duration-300 ${isDark ? 'text-violet-400' : 'text-slate-400'}`}>Total</span>
              <span className={`col-span-1 text-xs font-black uppercase tracking-wider text-center transition-colors duration-300 ${isDark ? 'text-violet-400' : 'text-slate-400'}`}>Fotos</span>
              <span className={`col-span-1 text-xs font-black uppercase tracking-wider text-center transition-colors duration-300 ${isDark ? 'text-violet-400' : 'text-slate-400'}`}>Quitar</span>
            </div>

            {lotes.map(l => (
              <div key={l.id} className={`rounded-2xl transition-all ${isDark ? l.cobro ? 'bg-violet-700/40 border border-violet-600' : 'bg-violet-900/20' : l.cobro ? 'bg-purple-50 border border-purple-100' : 'bg-slate-50'}`}>
                {/* Fila principal */}
                <div className="grid grid-cols-12 gap-2 px-4 py-3 items-center">
                  {/* Toggle cobro */}
                  <div className="col-span-1 flex justify-center">
                    <div onClick={() => toggleCobro(l.id)} className="cursor-pointer">
                      <div className={`w-5 h-5 rounded-full border-2 flex items-center justify-center transition-colors ${l.cobro ? isDark ? 'bg-violet-500 border-violet-500' : 'bg-purple-500 border-purple-500' : isDark ? 'border-violet-600 bg-[#3d2d52]' : 'border-slate-300 bg-white'}`}>
                  ...
                  {l.manual ? (
                    <input type="text" value={l.concepto} onChange={e => updateCampo(l.id, 'concepto', e.target.value)}
                      ...
                      className={`col-span-3 px-2 py-1.5 rounded-xl border font-bold text-sm bg-white transition-all outline-none ${isDark ? 'border-violet-600 text-violet-100 focus:ring-2 focus:ring-violet-400' : 'border-slate-200 focus:ring-2 focus:ring-purple-200'}`} />
                  ) : (
                    <span className={`col-span-3 text-sm font-bold truncate transition-colors duration-300 ${isDark ? 'text-violet-100' : 'text-slate-700'}`}>{l.concepto}</span>
                  )}
                  {/* Referencia */}
                  {l.manual ? (
                    <input type="text" value={l.referencia} onChange={e => updateCampo(l.id, 'referencia', e.target.value.toUpperCase())}
                      placeholder="Ref."
                      className={`col-span-1 px-2 py-1.5 rounded-xl border font-black text-sm text-center bg-white transition-all outline-none ${isDark ? 'border-violet-600 text-violet-500 focus:ring-2 focus:ring-violet-400' : 'border-slate-200 focus:ring-2 focus:ring-purple-200 text-purple-500'}`} />
                  ) : (
                    <span className={`col-span-1 text-sm font-black text-center leading-none transition-colors duration-300 ${isDark ? 'text-violet-400' : 'text-purple-500'}`}>{l.referencia}</span>
                  )}
                  {/* Vlr Unit */}
                  {l.manual ? (
                    <input type="number" min={0} value={l.vlrUnit || ''} onChange={e => updateCampo(l.id, 'vlrUnit', parseFloat(e.target.value) || 0)}
                      placeholder="Precio"
                      className={`col-span-2 px-2 py-1.5 rounded-xl border font-semibold text-sm text-right bg-white transition-all outline-none ${isDark ? 'border-violet-600 text-violet-100 focus:ring-2 focus:ring-violet-400' : 'border-slate-200 focus:ring-2 focus:ring-purple-200 text-slate-500'}`} />
                  ) : (
                    <span className={`col-span-2 text-[15px] font-semibold text-right leading-none transition-colors duration-300 ${isDark ? 'text-violet-400' : 'text-slate-500'}`}>{fmt(l.vlrUnit)}</span>
                  )}
                  {/* spacer */}
                  ...
                  <div className="col-span-1 flex justify-center">
                    <input type="number" min={1} value={l.unidades}
                      ...
                      className={`w-14 px-2 py-1.5 rounded-xl border font-bold text-sm text-center bg-white transition-all outline-none ${isDark ? 'border-violet-600 text-violet-100 focus:ring-2 focus:ring-violet-400' : 'border-slate-200 focus:ring-2 focus:ring-purple-200 text-slate-800'}`} />
                  </div>
                  {/* Total */}
                  <span className={`col-span-1 text-sm font-black text-right transition-colors duration-300 ${isDark ? l.cobro ? 'text-violet-200' : 'text-violet-300' : l.cobro ? 'text-purple-600' : 'text-slate-800'}`}>{fmt(l.total)}</span>
                  {/* Botones foto + ficha */}
                  <div className="col-span-1 flex justify-center gap-1">
                    {(state.fichasCosto || []).find((f: any) => f.referencia === l.referencia)?.foto1 && (
                      <button onClick={() => { const f = (state.fichasCosto || []).find((f: any) => f.referencia === l.referencia); if (f?.foto1) setFotoModal({ url: f.foto1, ref: l.referencia }); }}
                        className={`p-1 rounded-lg transition-colors border ${isDark ? 'bg-violet-900/40 hover:bg-violet-900/60 text-violet-400 border-violet-600' : 'bg-white hover:bg-purple-100 text-purple-400 border-slate-200'}`}>
                  ...
                  {/* Eliminar */}
                  <div className="col-span-1 flex justify-center">
                    <button onClick={() => eliminarLote(l.id)} className={`p-1 rounded-lg transition-colors ${isDark ? 'text-violet-600 hover:text-pink-400 hover:bg-violet-900/30' : 'text-slate-300 hover:text-red-400 hover:bg-red-50'}`}>
                      <svg xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24" strokeWidth={2} stroke="currentColor" className="w-4 h-4">
                ...
                {l.cobro && (
                  <div className={`grid grid-cols-12 gap-2 px-4 pb-3 items-center border-t transition-colors duration-300 ${isDark ? 'border-violet-600' : 'border-purple-100'}`}>
                    <div className="col-span-1" />
                    <span className={`col-span-3 text-xs font-black uppercase tracking-wider transition-colors duration-300 ${isDark ? 'text-violet-300' : 'text-purple-500'}`}>Und. a cobrar</span>
                    <div className="col-span-1" />
                    <span className={`col-span-2 text-xs font-semibold text-right transition-colors duration-300 ${isDark ? 'text-violet-400' : 'text-slate-400'}`}>{fmt(l.precioVenta + 500)} / und</span>
                    <div className="col-span-1 flex justify-center">
                      <input type="number" min={0} max={l.unidades} value={l.unidadesCobro}
                        ...
                        onFocus={e => e.target.select()}
                        className={`w-14 px-2 py-1.5 rounded-xl border font-bold text-sm text-center bg-white transition-all outline-none ${isDark ? 'border-violet-500 focus:ring-2 focus:ring-violet-300 text-violet-700' : 'border-purple-200 focus:ring-2 focus:ring-purple-300 text-purple-700'}`} />
                    </div>
                    <span className={`col-span-2 text-sm font-black text-right transition-colors duration-300 ${isDark ? 'text-violet-200' : 'text-purple-600'}`}>{fmt((l.precioVenta + 500) * l.unidadesCobro)}</span>
            ...
            {/* Total */}
            <div className={`flex justify-between items-center px-4 pt-3 border-t mt-2 transition-colors duration-300 ${isDark ? 'border-violet-700' : 'border-slate-100'}`}>
              <span className={`text-xs font-black uppercase tracking-widest transition-colors duration-300 ${isDark ? 'text-violet-400' : 'text-slate-400'}`}>Total lotes</span>
              <span className={`text-2xl font-black transition-colors duration-300 ${isDark ? 'text-violet-200' : 'text-slate-800'}`}>{fmt(totalLotes)}</span>
            </div>
            </>
          )}
        </div>
      ...
      {lotes.length > 0 && (
        <>
          <div className={`rounded-3xl p-6 shadow-lg flex items-center justify-between gap-4 transition-colors duration-300 ${isDark ? 'bg-gradient-to-r from-violet-600 to-purple-600' : 'bg-gradient-to-r from-purple-500 to-violet-400'}`}>
            <div>
              <p className={`text-xs font-black uppercase tracking-widest mb-1 transition-colors duration-300 ${isDark ? 'text-violet-100' : 'text-purple-100'}`}>Total neto a pagar</p>
          ...
          <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
            ...
            <div className={`rounded-3xl border shadow-sm overflow-hidden flex flex-col transition-colors duration-300 ${isDark ? 'bg-[#4a3a63] border-violet-700' : 'bg-white border-slate-100'}`}>
              <div className={`px-6 py-3 border-b flex items-center justify-center gap-2 transition-colors duration-300 ${isDark ? 'bg-violet-900/40 border-violet-600' : 'bg-emerald-50 border-emerald-100'}`}>
                <div className={`w-7 h-7 rounded-xl flex items-center justify-center transition-colors duration-300 ${isDark ? 'bg-violet-700/40' : 'bg-emerald-100'}`}><span className={`text-xs font-black transition-colors duration-300 ${isDark ? 'text-violet-300' : 'text-emerald-600'}`}>ML</span></div>
              ...
              <div className="p-6 flex flex-col flex-1">
                ...
                <div className={`mt-auto pt-3 border-t space-y-2 transition-colors duration-300 ${isDark ? 'border-violet-700' : 'border-slate-100'}`}>
                  <div className="flex justify-between items-center" style={{ visibility: totalCobro > 0 ? 'visible' : 'hidden' }}>
                    <span className={`text-xs font-semibold transition-colors duration-300 ${isDark ? 'text-violet-400' : 'text-slate-400'}`}>Cobro ({lotes.filter(l => l.cobro).length} lote{lotes.filter(l => l.cobro).length !== 1 ? 's' : ''})</span>
                    <span className={`text-sm font-black transition-colors duration-300 ${isDark ? 'text-pink-400' : 'text-red-400'}`}>− {fmt(totalCobro)}</span>
                  </div>
                  ...
                  <div className={`flex justify-between items-center rounded-xl px-3 py-2 transition-colors duration-300 ${isDark ? 'bg-violet-900/40' : 'bg-emerald-50'}`}>
                    ...
                    <span className={`text-base font-black transition-colors duration-300 ${isDark ? 'text-violet-200' : 'text-emerald-700'}`}>{fmt(totalMLNeto)}</span>
                  </div>
                </div>
              </div>
            </div>
          ...
          <div className={`rounded-3xl border shadow-sm overflow-hidden transition-colors duration-300 ${isDark ? 'bg-[#4a3a63] border-violet-700' : 'bg-white border-slate-100'}`}>
            ...
            <div className="p-6 space-y-3">
              <div className={`flex justify-between items-center py-3 px-4 rounded-2xl transition-colors duration-300 ${isDark ? 'bg-violet-900/40' : 'bg-blue-50'}`}>
                <div className="flex items-center gap-2">
                  <div className={`w-7 h-7 rounded-lg flex items-center justify-center transition-colors duration-300 ${isDark ? 'bg-violet-700/40' : 'bg-blue-100'}`}><span className={`text-xs font-black transition-colors duration-300 ${isDark ? 'text-violet-300' : 'text-blue-600'}`}>OF</span></div>
              ...
              <div className={`flex justify-between items-center py-3 px-4 rounded-2xl transition-colors duration-300 ${isDark ? 'bg-violet-900/40' : 'bg-emerald-50'}`}>
                <div className="flex items-center gap-2">
                  <div className={`w-7 h-7 rounded-lg flex items-center justify-center transition-colors duration-300 ${isDark ? 'bg-violet-700/40' : 'bg-emerald-100'}`}><span className={`text-xs font-black transition-colors duration-300 ${isDark ? 'text-violet-300' : 'text-emerald-600'}`}>ML</span></div>
        ...
        <div className={`fixed inset-0 flex items-center justify-center z-50 p-4 transition-colors duration-300 ${isDark ? 'bg-black/60 backdrop-blur-sm' : 'bg-black/40 backdrop-blur-sm'}`}>
          <div className={`rounded-3xl shadow-2xl max-w-sm w-full p-8 transition-colors duration-300 ${isDark ? 'bg-[#4a3a63]' : 'bg-white'}`}>
            ...
            <div className="space-y-4">
              <div>
                <label className={`text-xs font-bold uppercase tracking-wider block mb-1.5 transition-colors duration-300 ${isDark ? 'text-violet-300' : 'text-slate-500'}`}>Fecha de proceso</label>
                <input type="date" value={fechaLlegada}
                  ...
                  className={`w-full px-4 py-3 rounded-2xl border font-bold text-sm transition-all outline-none ${isDark ? 'bg-[#3d2d52] border-violet-600 text-violet-100 focus:ring-2 focus:ring-violet-400 focus:border-violet-500' : 'border-slate-200 focus:ring-2 focus:ring-purple-200 focus:border-purple-400 text-slate-800'}`} />
              </div>
              <div>
                <label className={`text-xs font-bold uppercase tracking-wider block mb-1.5 transition-colors duration-300 ${isDark ? 'text-violet-300' : 'text-slate-500'}`}>
                  Fecha sugerida de pago <span className={`font-normal transition-colors duration-300 ${isDark ? 'text-violet-400' : 'text-purple-400'}`}>(+7 días)</span>
                </label>
                <input type="date" value={fechaSugerida} onChange={e => setFechaSugerida(e.target.value)}
                  className={`w-full px-4 py-3 rounded-2xl border-2 font-bold text-sm transition-all outline-none ${isDark ? 'bg-[#3d2d52] border-violet-500 text-violet-100 focus:ring-2 focus:ring-violet-400 focus:border-violet-400' : 'border-purple-200 focus:ring-2 focus:ring-purple-200 focus:border-purple-400 text-purple-700'}`} />
              </div>
              <div className={`rounded-2xl p-4 space-y-2 transition-colors duration-300 ${isDark ? 'bg-violet-900/40 text-violet-300' : 'bg-purple-50 text-purple-600'}`}>
                <p className={`text-center font-bold text-sm transition-colors duration-300 ${isDark ? 'text-violet-200' : ''}`}>{`REF. ${[...new Set(lotes.map(l => l.referencia))].join(' - ')}`}</p>
                <div className="flex gap-3">
                  <div className={`flex-1 rounded-xl p-2 text-center transition-colors duration-300 ${isDark ? 'bg-[#3d2d52]' : 'bg-white'}`}>
                    <p className={`font-semibold mb-0.5 transition-colors duration-300 ${isDark ? 'text-violet-400' : 'text-purple-400'}`}>Bruto OF</p>
                    <p className={`font-black transition-colors duration-300 ${isDark ? 'text-violet-200' : 'text-purple-700'}`}>{fmt(Math.round(valorOF))}</p>
                  ...
                  <div className={`flex-1 rounded-xl p-2 text-center transition-colors duration-300 ${isDark ? 'bg-[#3d2d52]' : 'bg-white'}`}>
                    <p className={`font-semibold mb-0.5 transition-colors duration-300 ${isDark ? 'text-violet-400' : 'text-purple-400'}`}>Bruto ML</p>
                    <p className={`font-black transition-colors duration-300 ${isDark ? 'text-violet-200' : 'text-purple-700'}`}>{fmt(Math.round(valorML))}</p>
                    {totalCobro > 0 && <p className={`text-xs mt-0.5 transition-colors duration-300 ${isDark ? 'text-pink-400' : 'text-red-400'}`}>COBRO: -{fmt(Math.round(totalCobro))}</p>}
            ...
            <div className="flex gap-3 mt-6">
              ...
              <button onClick={handleAsentar} disabled={!fechaSugerida}
                className={`flex-1 font-black py-3 rounded-2xl transition-colors disabled:opacity-40 ${isDark ? 'bg-violet-600 hover:bg-violet-700 text-white' : 'bg-purple-500 hover:bg-purple-600 text-white'}`}>
                Asentar
              </button>
            </div>
          </div>
        ...
        <div className={`fixed inset-0 flex items-center justify-center z-50 p-4 transition-colors duration-300 ${isDark ? 'bg-black/60 backdrop-blur-sm' : 'bg-black/40 backdrop-blur-sm'}`}>
          <div className={`rounded-3xl shadow-2xl w-full max-w-2xl p-6 flex flex-col gap-4 max-h-[90vh] transition-colors duration-300 ${isDark ? 'bg-[#4a3a63]' : 'bg-white'}`}>
            <div className="flex items-center justify-between">
              <h3 className={`font-black text-xl transition-colors duration-300 ${isDark ? 'text-violet-50' : 'text-slate-800'}`}>Transportes — {referencia || 'Lotes'}</h3>
      ...
      {/* Modal foto */}
      {fotoModal && (
        <div className={`fixed inset-0 flex items-center justify-center z-50 p-4 transition-colors duration-300 ${isDark ? 'bg-black/80' : 'bg-black/60'}`} onClick={() => setFotoModal(null)}>
          <div className={`rounded-3xl overflow-hidden shadow-2xl max-w-sm w-full transition-colors duration-300 ${isDark ? 'bg-[#4a3a63]' : 'bg-white'}`} onClick={e => e.stopPropagation()}>
            <div className={`flex items-center justify-between px-5 py-4 border-b transition-colors duration-300 ${isDark ? 'border-violet-700' : 'border-slate-100'}`}>
              <p className={`font-black text-sm transition-colors duration-300 ${isDark ? 'text-violet-200' : 'text-slate-700'}`}>{fotoModal.ref}</p>
              <button onClick={() => setFotoModal(null)} className={`p-1 rounded-lg transition-colors ${isDark ? 'hover:bg-violet-900/40' : 'hover:bg-slate-100'}`}>
                <svg xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24" strokeWidth={2.5} stroke="currentColor" className={`w-5 h-5 transition-colors duration-300 ${isDark ? 'text-violet-400' : 'text-slate-400'}`}>
                  <path strokeLinecap="round" strokeLinejoin="round" d="M6 18L18 6M6 6l12 12" />
                </svg>
              </button>
            </div>
            <img src={fotoModal.url} alt={fotoModal.ref} className="w-full object-contain max-h-[70vh]" />
            <p className={`text-center text-xs py-2 transition-colors duration-300 ${isDark ? 'text-violet-400' : 'text-slate-400'}`}>Presiona ESC para cerrar</p>
          </div>
        </div>
      )}
            ...
            <div className="space-y-4">
              ...
              <div>
                ...
                <div className="relative">
                  ...
                  <input type="number" value={baseRte} onChange={e => setBaseRte(Number(e.target.value))} onFocus={e => e.target.select()} className={`w-full pl-8 pr-4 py-3 rounded-2xl border font-bold text-sm transition-all outline-none ${isDark ? 'bg-[#3d2d52] border-violet-600 text-violet-100 focus:ring-2 focus:ring-violet-400 focus:border-violet-500' : 'border-slate-200 focus:ring-2 focus:ring-purple-200 text-slate-800'}`} />
                </div>
              </div>
            </div>
