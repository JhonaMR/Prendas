...
function calcProduccion(row: TelaProduccion): TelaProduccion {
  ...
  if (row.undMedida === 'M') {
    return { ...row, iva: String(iva), precioTotalKilos: '', precioTotalMetros: String(neto + iva) };
  }
  return {
    ...row, iva: String(iva),
...
function calcMuestra(row: TelaMuestra): TelaMuestra {
  ...
  const totalMetros = rdmto > 0 ? parseFloat((totalKilos / rdmto).toFixed(2)) : 0;
  if (row.undMedida === 'M') {
    return { ...row, iva: String(iva), totalPrecioKilos: '', totalPrecioMetros: String(neto + iva) };
  }
  return {
    ...row, iva: String(iva),
...
const UndBadge = ({ u }: { u: string }) => (
  <span className={`px-2 py-0.5 rounded-full text-xs font-bold ${u === 'K' ? 'bg-blue-100 text-blue-700' : 'bg-purple-100 text-purple-700'}`}>{u}</span>
);
...
const FilterBar: React.FC<{
  ...
  onClear: () => void;
}> = ({ filterTela, setFilterTela, filterProveedor, setFilterProveedor, onClear }) => (
  <div className="flex items-center gap-2">
    <button onClick={onClear} className="flex items-center justify-center w-9 h-9 rounded-xl border border-slate-200 bg-white text-slate-400 hover:text-red-500 hover:border-red-200 transition-all flex-shrink-0" title="Limpiar filtros">
    ...
    <div className="relative">
      <input type="text" placeholder="Buscar tela..." value={filterTela} onChange={e => setFilterTela(e.target.value)}
        className="pl-8 pr-3 py-2 rounded-xl border border-slate-200 bg-white text-sm text-slate-700 placeholder-slate-400 focus:outline-none focus:ring-2 focus:ring-slate-300 w-44" />
      <svg xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24" strokeWidth={2} stroke="currentColor" className="w-3.5 h-3.5 text-slate-400 absolute left-2.5 top-1/2 -translate-y-1/2 pointer-events-none">
    ...
    <div className="relative">
      <input type="text" placeholder="Buscar proveedor..." value={filterProveedor} onChange={e => setFilterProveedor(e.target.value)}
        className="pl-8 pr-3 py-2 rounded-xl border border-slate-200 bg-white text-sm text-slate-700 placeholder-slate-400 focus:outline-none focus:ring-2 focus:ring-slate-300 w-48" />
      <svg xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24" strokeWidth={2} stroke="currentColor" className="w-3.5 h-3.5 text-slate-400 absolute left-2.5 top-1/2 -translate-y-1/2 pointer-events-none">
        <path strokeLinecap="round" strokeLinejoin="round" d="m21 21-5.197-5.197m0 0A7.5 7.5 0 1 0 5.196 5.196a7.5 7.5 0 0 0 10.607 10.607Z" />
      </svg>
    </div>
...
// ── Toggle pills ───────────────────────────────────────────────────────────────
const UndToggle: React.FC<{ value: string; onChange: (v: string) => void }> = ({ value, onChange }) => (
  <div className="flex rounded-full overflow-hidden border border-slate-200 w-fit mx-auto">
...
const IvaToggle: React.FC<{ value: string; onChange: (v: string) => void }> = ({ value, onChange }) => (
  <div className="flex rounded-full overflow-hidden border border-slate-200 w-fit mx-auto">
...
const TelaProduccionTable: React.FC<{ onBack: () => void; user: User }> = ({ onBack, user }) => {
  ...
  const thCls = 'px-3 py-3 text-xs font-black text-center text-pink-400 uppercase tracking-wider whitespace-nowrap';
  const tdCls = 'px-3 py-2 text-[13px]';
  const inputCls = 'w-full px-2 py-1 text-[13px] border-none outline-none bg-transparent focus:ring-1 focus:ring-inset focus:ring-pink-300 focus:rounded-sm';
    ...
    <div className="space-y-4 pb-20">
      ...
      <div className="flex items-center justify-between gap-4">
        <div className="flex items-center gap-4">
          <button onClick={onBack} className="px-5 py-3 rounded-2xl bg-white text-slate-400 font-bold hover:text-slate-600 transition-all border border-slate-100 text-sm">← Atrás</button>
          <div>
            <h2 className="text-3xl font-black text-slate-800 tracking-tighter">Telas para producción</h2>
            {hasUnsaved && (
              <p className="text-slate-400 font-medium text-sm">
                <span className="flex items-center gap-1.5 text-amber-600 font-semibold"><span className="w-2 h-2 rounded-full bg-amber-500 animate-pulse inline-block" />Cambios sin guardar</span>
              </p>
            )}
          </div>
        </div>
      ...
      {loading ? (
        <div className="flex justify-center py-20"><div className="w-8 h-8 border-4 border-pink-400 border-t-transparent rounded-full animate-spin" /></div>
      ) : (
...
const TelaMuestrasTable: React.FC<{ onBack: () => void; user: User }> = ({ onBack, user }) => {
  ...
  const thCls = 'px-3 py-3 text-xs font-black text-center text-purple-400 uppercase tracking-wider whitespace-nowrap';
  const tdCls = 'px-3 py-2 text-[13px]';
  const inputCls = 'w-full px-2 py-1 text-[13px] border-none outline-none bg-transparent focus:ring-1 focus:ring-inset focus:ring-pink-300 focus:rounded-sm';
    ...
    <div className="space-y-4 pb-20">
      <div className="flex items-center justify-between gap-4">
        <div className="flex items-center gap-4">
          <button onClick={onBack} className="px-5 py-3 rounded-2xl bg-white text-slate-400 font-bold hover:text-slate-600 transition-all border border-slate-100 text-sm">← Atrás</button>
          <div>
            <h2 className="text-3xl font-black text-slate-800 tracking-tighter">Telas para muestras</h2>
            {hasUnsaved && (
              <p className="text-slate-400 font-medium text-sm">
                <span className="flex items-center gap-1.5 text-amber-600 font-semibold"><span className="w-2 h-2 rounded-full bg-amber-500 animate-pulse inline-block" />Cambios sin guardar</span>
              </p>
      ...
      {loading ? (
        <div className="flex justify-center py-20"><div className="w-8 h-8 border-4 border-purple-400 border-t-transparent rounded-full animate-spin" /></div>
      ) : (
        <div className="bg-white rounded-3xl shadow-sm border border-slate-100 overflow-hidden">
          <div className="overflow-x-auto">
            <table className="w-full text-sm table-fixed">
    ...
    <div className="space-y-8 pb-20">
      <div className="flex items-center justify-between">
        <div>
          ...
          <p className="text-slate-400 font-medium">Selecciona el tipo de control</p>
        </div>
        <button onClick={() => setBuscarOpen(true)}
          className="flex items-center gap-2 px-5 py-2.5 rounded-2xl bg-gradient-to-r from-pink-500 to-purple-500 text-white font-bold text-sm hover:from-pink-600 hover:to-purple-600 shadow-md hover:shadow-lg transition-all">
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 gap-8">
        <button onClick={() => setView('produccion')} className="bg-white p-8 rounded-[32px] shadow-sm border border-slate-100 hover:shadow-md hover:border-pink-200 transition-all flex flex-col items-center justify-center gap-6 min-h-[400px]">
          ...
          <div className="text-center space-y-2">
            <h3 className="text-2xl font-black text-slate-800">Telas para producción</h3>
            <p className="text-slate-400 font-medium">Control de telas destinadas a producción</p>
          </div>
        </button>
        <button onClick={() => setView('muestras')} className="bg-white p-8 rounded-[32px] shadow-sm border border-slate-100 hover:shadow-md hover:border-purple-200 transition-all flex flex-col items-center justify-center gap-6 min-h-[400px]">
          ...
          <div className="text-center space-y-2">
            <h3 className="text-2xl font-black text-slate-800">Telas para muestras</h3>
            <p className="text-slate-400 font-medium">Control de telas destinadas a muestras</p>
          </div>
        </button>
      </div>
    </div>
