// ============================================
// VISTA: Histórico de Fichas de Confección
...
interface Props {
    ...
    fichas: FichaConfeccionRecord[];
    onEliminar: (id: string) => void;
}

const HistoricoFichasConfeccion: React.FC<Props> = ({ user, onNavigate, fichas, onEliminar }) => {
    ...
    return (
        <div className={`space-y-6 pb-20 transition-colors ${isDark ? 'bg-[#3d2d52]' : 'bg-white'}`}>
            ...
            <div className="flex items-center justify-between">
                ...
                <button
                    onClick={() => onNavigate('ficha-confeccion-editor', {})}
                ...
                <table className="w-full">
                    <thead>
                        <tr className={`border-b ${isDark ? 'bg-[#4a3a63] border-violet-700' : 'bg-slate-50 border-slate-200'}`}>
                            {['Referencia', 'Cantidad Cortada', 'Fecha Creación', 'Realizado Por', 'Acciones'].map(h => (
                    ...
                    <tbody className={`divide-y ${isDark ? 'divide-violet-800' : 'divide-slate-100'}`}>
                        {filtradas.length === 0 ? (
                            <tr>
                                <td colSpan={5} className={`px-6 py-16 text-center font-bold text-sm ${isDark ? 'text-violet-500' : 'text-slate-400'}`}>
                            ...
                            <tr key={f.id} className={`transition-colors ${isDark ? 'hover:bg-violet-900/20' : 'hover:bg-slate-50'}`}>
                                ...
                                <td className="px-6 py-4">
                                    <div className="flex items-center gap-2">
                                        <button
                                            onClick={() => onNavigate('ficha-confeccion-editor', { fichaId: f.id })}
...
export default HistoricoFichasConfeccion;
