...
const AdminLayout: React.FC<AdminLayoutProps> = ({ user, onNavigate, onDirectNavigate, state, correrias, correriasLoading, correriasError }) => {
  ...
  const navGroups: NavGroup[] = [
    {
      ...
      items: [
        ...
        { id: 'settle', label: 'Asentar Ventas', icon: <Icons.Settle />, description: 'Registrar ventas' },
        { id: 'orderHistory', label: 'Historial de Pedidos', icon: <Icons.History />, description: 'Consultar historial' },
        { id: 'listaPrecios', label: 'Toma de pedidos manual', icon: <Icons.ListaPrecios />, description: 'Generar lista de precios y toma de pedidos manual' },
      ],
    },
    {
      id: 'informes',
    ...
    },
    {
      ...
      items: [
        ...
        { id: 'asistencia', label: 'Control de Asistencia', icon: <Icons.Asistencia />, description: 'Control de asistencia del personal' },
      ],
    },
    {
      id: 'admin',
  ...
  const filteredNavigationItems = navigationItems.filter(item => {
    if (['asistencia', 'fichas-confeccion', 'fichas-estampacion'].includes(item.id)) {
  ...
  const currentGroup = activeGroup ? filteredNavGroups.find(g => g.id === activeGroup) : null;
    ...
    <div className="h-full w-full flex flex-col bg-transparent p-6 md:p-10">
        ...
        <div className="grid grid-cols-2 sm:grid-cols-3 gap-3 md:gap-4 mb-8" style={{ gridAutoRows: '8rem' }}>
          {filteredNavGroups.map((group) => (
            <button
              ...
              className={`group relative h-full rounded-2xl border-2 transition-all duration-300 p-4 flex flex-col items-start justify-between overflow-hidden ${isDark
            ...
            >
              ...
              <div className={`absolute inset-y-3 left-1/2 -translate-x-1/2 w-px ${isDark ? 'bg-violet-600' : 'bg-slate-200'
                }`} />
              <div className="relative z-10 flex h-full w-full gap-3">
                ...
                <div className="w-1/2 flex flex-col items-start justify-between pr-3">
                  <div className={`w-9 h-9 md:w-11 md:h-11 rounded-xl flex items-center justify-center transition-colors ${isDark
                    ? 'bg-pink-900/40 text-pink-400 group-hover:bg-pink-900/60'
                    : 'bg-pink-100 text-pink-600 group-hover:bg-pink-200'
                    }`}>
                    {group.icon}
                  </div>
                  <h3 className={`font-bold text-sm md:text-base group-hover:text-pink-600 transition-colors leading-tight ${isDark ? 'text-violet-50' : 'text-slate-900'
                    }`}>
                    {group.label}
                  </h3>
                </div>
                ...
                <div className="w-1/2 flex flex-col justify-start gap-1 pl-3 pt-1">
                  {group.items.map(item => (
                    <div key={item.id} className="flex items-center gap-1.5">
                      <span className={`w-1 h-1 rounded-full flex-shrink-0 ${isDark ? 'bg-violet-600' : 'bg-slate-300'
                        }`} />
                      <span className={`text-xs leading-tight truncate ${isDark ? 'text-violet-200' : 'text-slate-400'
                        }`}>
                        {item.label}
                      </span>
                    </div>
              ...
              <div className="absolute top-3 right-3 opacity-0 group-hover:opacity-100 transition-opacity duration-300">
                <svg xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24" strokeWidth={2.5} stroke="currentColor" className={`w-4 h-4 ${isDark ? 'text-pink-500' : 'text-pink-500'
                  }`}>
          ...
          {currentGroup.items.map((item) => (
            ...
            >
              ...
              <div className="relative z-10 flex flex-col items-start justify-between h-full w-full">
                <div className={`w-8 h-8 md:w-10 md:h-10 rounded-lg flex items-center justify-center transition-colors ${isDark
                  ? 'bg-pink-900/40 text-pink-400 group-hover:bg-pink-900/60'
                  : 'bg-pink-100 text-pink-600 group-hover:bg-pink-200'
                  }`}>
                  {item.icon}
                ...
                <div className="text-left">
                  <h3 className={`font-bold text-xs md:text-sm group-hover:text-pink-600 transition-colors leading-tight ${isDark ? 'text-violet-50' : 'text-slate-900'
                    }`}>
                    {item.label}
                  </h3>
                </div>
              ...
              <div className="absolute top-2 right-2 opacity-0 group-hover:opacity-100 transition-opacity duration-300">
                <svg xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24" strokeWidth={2.5} stroke="currentColor" className={`w-4 h-4 ${isDark ? 'text-pink-500' : 'text-pink-600'
                  }`}>
                  <path strokeLinecap="round" strokeLinejoin="round" d="M13.5 4.5L21 12m0 0l-7.5 7.5M21 12H3" />
                </svg>
              </div>
      ...
      {!currentGroup && (
        <div className="flex-1 flex flex-col gap-6">
          {error && (
            <div className={`border rounded-2xl p-4 flex items-start gap-3 ${isDark
              ? 'bg-red-900/30 border-red-700'
              : 'bg-red-50 border-red-200'
              }`}>
              <svg xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24" strokeWidth={2} stroke="currentColor" className={`w-5 h-5 flex-shrink-0 mt-0.5 ${isDark ? 'text-red-400' : 'text-red-600'
                }`}>
                <path strokeLinecap="round" strokeLinejoin="round" d="M12 9v3.75m9-.75a9 9 0 11-18 0 9 9 0 0118 0zm-9 3.75h.008v.008H12v-.008z" />
              </svg>
              <p className={`text-sm font-semibold ${isDark ? 'text-red-300' : 'text-red-900'
                }`}>{error}</p>
            </div>
          )}
          ...
          {!error && (
            <>
              <div className={`rounded-3xl shadow-sm border p-6 ${isDark
                ? 'bg-[#4a3a63] border-violet-700'
                : 'bg-white border-slate-200'
                }`}>
                <div className="flex items-center justify-between mb-3">
                  <h3 className={`text-sm font-bold uppercase tracking-wide ${isDark ? 'text-violet-200' : 'text-slate-500'
                    }`}>Análisis rápido de correrías</h3>
                  {selectedCorreria && (
                    <button
                      onClick={() => setSelectedCorreria(null)}
                      className={`w-7 h-7 flex items-center justify-center rounded-full transition-colors ${isDark
                        ? 'bg-violet-900/40 hover:bg-violet-900/60'
                        : 'bg-slate-100 hover:bg-slate-200'
                        }`}
                      aria-label="Limpiar selección"
                    >
                      <svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth={2.5} strokeLinecap="round" strokeLinejoin="round" className={`w-3.5 h-3.5 ${isDark ? 'text-violet-500' : 'text-slate-500'
                        }`}>
      ...
      <ViewOrderModal
        ...
        availableViews={filteredNavigationItems}
