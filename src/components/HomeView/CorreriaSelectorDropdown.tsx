  ...
  return (
    <div ref={dropdownRef} className="relative w-full">
      ...
      <button
        ...
        className={`w-full px-4 py-3 border-2 rounded-2xl flex items-center justify-between transition-colors disabled:opacity-50 disabled:cursor-not-allowed ${
          isDark
            ? 'bg-[#3d2d52] border-violet-600 text-violet-100 hover:border-violet-500'
            : 'bg-white border-slate-200 text-slate-700 hover:border-blue-500'
        }`}
      >
        <span className="font-medium text-left">{selectedCorreriaName}</span>
        <svg
          ...
          className={`w-5 h-5 transition-transform ${isOpen ? 'rotate-180' : ''} ${
            isDark ? 'text-violet-500' : 'text-slate-400'
          }`}
        >
      ...
      {/* Dropdown Menu */}
      {isOpen && (
        <div className={`absolute top-full left-0 right-0 mt-2 border-2 rounded-2xl shadow-lg z-50 ${
          isDark
          ...
          <div className={`p-3 border-b ${
            ...
            <input
              ...
              className={`w-full px-3 py-2 border rounded-lg text-sm focus:outline-none transition-colors ${
                isDark
                  ? 'bg-[#3d2d52] border-violet-600 text-violet-100 placeholder-violet-500 focus:border-violet-500'
          ...
          <div className="max-h-64 overflow-y-auto custom-scrollbar">
            {filteredCorrerias.length > 0 ? (
                ...
                <button
                  ...
                  className={`w-full px-4 py-3 text-left transition-colors flex items-center justify-between ${
                    selectedCorreria === correria.id
                      ...
                      : isDark
                      ? 'text-violet-200 hover:bg-violet-700/20'
              ...
              <div className={`px-4 py-6 text-center text-sm ${
                isDark ? 'text-violet-400' : 'text-slate-500'
              }`}>
