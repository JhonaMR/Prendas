...
import { Compra } from '../views/ComprasView';
import api from '../services/api';

interface ComprasImportModalProps {
  isOpen: boolean;
  onClose: () => void;
  onImport: (data: Compra[]) => void;
}
...
const ComprasImportModal: React.FC<ComprasImportModalProps> = ({
  isOpen,
  onClose,
  onImport
}) => {
  ...
  const validateRow = (row: any, rowIndex: number): { valid: boolean; errors: string[] } => {
    ...
    // Validar fecha
    const fecha = parseExcelDate(getColumnValue(row, ['FECHA']));
    if (!fecha) {
      errors.push('Fecha requerida');
    }

    // Validar insumo
    const insumo = getColumnValue(row, ['INSUMO']);
    if (!insumo || String(insumo).trim() === '') {
      errors.push('Insumo requerido');
    }

    // Validar cantidad insumo
    const cantidadInsumo = Number(getColumnValue(row, ['CANTIDAD INSUMO']));
    if (!cantidadInsumo || cantidadInsumo <= 0) {
      errors.push('Cantidad Insumo debe ser mayor a 0');
    }

    // Validar proveedor
    const proveedor = getColumnValue(row, ['PROVEEDOR']);
    if (!proveedor || String(proveedor).trim() === '') {
      errors.push('Proveedor requerido');
    }
  ...
  const handleFileSelect = async (event: React.ChangeEvent<HTMLInputElement>) => {
    ...
    try {
      ...
      rows.forEach((row: any, index: number) => {
        ...
        if (validation.valid) {
          const fecha = parseExcelDate(getColumnValue(row, ['FECHA']));
          const fechaPedido = getColumnValue(row, ['FECHA DE PEDIDO']) 
            ? parseExcelDate(getColumnValue(row, ['FECHA DE PEDIDO'])) 
            : null;

          const compraRecord: Compra = {
            id: `import_${Date.now()}_${index}`,
            fecha: fecha || '',
            referencia: getColumnValue(row, ['REFERENCIA']) ? String(getColumnValue(row, ['REFERENCIA'])).trim() : null,
            unidades: getColumnValue(row, ['UNIDADES']) ? Number(getColumnValue(row, ['UNIDADES'])) : null,
            insumo: String(getColumnValue(row, ['INSUMO'])).trim(),
            cantidadInsumo: Number(getColumnValue(row, ['CANTIDAD INSUMO'])),
            precioUnidad: getColumnValue(row, ['PRECIO UND']) ? Number(getColumnValue(row, ['PRECIO UND'])) : 0,
            cantidadTotal: getColumnValue(row, ['CANTIDAD TOTAL']) ? Number(getColumnValue(row, ['CANTIDAD TOTAL'])) : Number(getColumnValue(row, ['CANTIDAD INSUMO'])),
            total: getColumnValue(row, ['TOTAL']) ? Number(getColumnValue(row, ['TOTAL'])) : 0,
            proveedor: String(getColumnValue(row, ['PROVEEDOR'])).trim(),
            fechaPedido: fechaPedido,
            observacion: getColumnValue(row, ['OBSERVACION']) ? String(getColumnValue(row, ['OBSERVACION'])).trim() : null,
            factura: getColumnValue(row, ['FACTURA']) ? String(getColumnValue(row, ['FACTURA'])).trim() : null,
            precioRealInsumoUnd: 'pendiente',
            afectaInventario: false
          };

          valid.push(compraRecord);
        } else {
          invalid.push({
            row: index + 2,
            data: row,
    ...
    <div className="fixed inset-0 bg-black bg-opacity-50 flex items-center justify-center z-50 p-4">
      <div className="bg-white rounded-3xl shadow-2xl max-w-4xl w-full max-h-[90vh] overflow-y-auto">
        ...
        <div className="sticky top-0 bg-gradient-to-r from-blue-600 to-indigo-600 px-8 py-6 flex justify-between items-center border-b border-blue-700">
          <div>
            <h2 className="text-2xl font-black text-white">Importar Compras</h2>
            ...
            <div className="space-y-6">
              ...
              <div className="bg-blue-50 border border-blue-200 rounded-xl p-4">
                <p className="text-sm text-blue-800">
                  ...
                  FECHA, REFERENCIA, UNIDADES, INSUMO, CANTIDAD INSUMO, PRECIO UND, 
                  CANTIDAD TOTAL, TOTAL, PROVEEDOR, FECHA DE PEDIDO, OBSERVACION, FACTURA
                </p>
              </div>
            </div>
          ) : (
                    ...
                    <table className="w-full text-xs">
                      <thead className="bg-slate-200 sticky top-0">
                        <tr>
                          ...
                          <th className="px-3 py-2 text-left font-bold">Insumo</th>
                          <th className="px-3 py-2 text-center font-bold">Cant. Insumo</th>
                          <th className="px-3 py-2 text-left font-bold">Proveedor</th>
                      ...
                      <tbody className="divide-y divide-slate-200">
                        {preview.valid.slice(0, 10).map((record, idx) => (
                          <tr key={idx} className="hover:bg-slate-100">
                            ...
                            <td className="px-3 py-2">{record.insumo}</td>
                            <td className="px-3 py-2 text-center">{record.cantidadInsumo}</td>
                            <td className="px-3 py-2">{record.proveedor}</td>
                            <td className="px-3 py-2">{record.referencia || '-'}</td>
                            <td className="px-3 py-2 text-center">${record.total.toLocaleString()}</td>
                          </tr>
                        ))}
                      </tbody>
                    </table>
...
export default ComprasImportModal;
